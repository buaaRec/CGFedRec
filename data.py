...
class SampleGenerator(object):
    ...
    def __init__(self, ratings, neg_num, evaluate):
        """
        args:
    ...
    def _sample_negative(self, ratings):
        ...
        interact_status['negative_samples'] = interact_status['negative_items'].apply(lambda x: random.sample(x, self.neg_num)) #sample 198 negative items
...
#         create validation data"""
        if self.evaluate == 'part':
            ...
            # assert (len(row.negative_samples) / 2)) * len(val_users)) == len(negative_users)
        ...
        elif self.evaluate == 'all':
            val_ratings = pd.merge(self.val_ratings, self.negatives[['userId', 'negative_items']], on='userId')
            val_users, val_items, negative_users, negative_items = [], [], [], []
            for row in val_ratings.itertuples():
                val_users.append(int(row.userId))
                val_items.append(int(row.itemId))
                for i in range(int(len(row.negative_items))):
                    negative_users.append(int(row.userId))
                    negative_items.append(int(list(row.negative_items)[i]))
            assert len(val_users) == len(val_items)
            assert len(negative_users) == len(negative_items)
            # assert 99 * len(val_users) == len(negative_users)
            assert val_users == sorted(val_users)
            # return [val_users, val_items, negative_users, negative_items]
            return [torch.LongTensor(val_users), torch.LongTensor(val_items), torch.LongTensor(negative_users),
                    torch.LongTensor(negative_items)]
    ...
    def test_data(self):
        ...
        if self.evaluate == 'part':
            ...
            test_ratings = pd.merge(self.test_ratings, self.negatives[['userId', 'negative_items']], on='userId')
            test_users, test_items, negative_users, negative_items = [], [], [], []
            for row in test_ratings.itertuples():
                test_users.append(int(row.userId))
                test_items.append(int(row.itemId))
            ...
            # assert 99 * len(val_users) == len(negative_users)
            assert test_users == sorted(test_users)
            # return [test_users, test_items, negative_users, negative_items]
            return [torch.LongTensor(test_users), torch.LongTensor(test_items), torch.LongTensor(negative_users),
                    torch.LongTensor(negative_items)]