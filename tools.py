...
def aggregateByComposite(client_params, graph_matrix, config):
    ...
    for user in client_params.keys():
        ...
        for neighbor_user in client_params.keys():
            neighbor_param = client_params[neighbor_user]['embedding_item.weight'].data
            neighbor_weight = aggregation_weight_vector[neighbor_user]
            aggregated_client_params[user]['embedding_item.weight'].data += neighbor_weight * neighbor_param
...
def calculate_l2_similarity(client_params, init_param):
    ...
    init_params = pca.fit_transform(init_param['embedding_item.weight'].data.cpu().numpy())
    for i in range(len(client_id)):
        user_param_i = client_params[client_id[i]]['embedding_item.weight'].data.cpu().numpy()
        # get user params
        ...
        for j in range(i, len(client_id)):
            user_param_j = client_params[client_id[j]]['embedding_item.weight'].data.cpu().numpy()
...
def get_principal(client_train_data, client_param_dict, k_principal):
    ...
    client_item_embedding = client_param_dict['embedding_item.weight']
...
def weight_client_server(user, client_model_params, server_model_param, map, model_dict, config):
    ...
    client_items_data = client_model_params[user]['embedding_item.weight'].data

    if user in map:
        server_items_data = server_model_param[map[user]]['embedding_item.weight'].data
        client_param_dict['embedding_item.weight'].data = config['interpolation'] * client_items_data + (
                    1 - config['interpolation']) * server_items_data
    else:
        client_param_dict['embedding_item.weight'].data = client_items_data
